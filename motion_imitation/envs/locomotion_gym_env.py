...
class LocomotionGymEnv(gym.Env):
  ...
  def reset(self,
    ...
    if self._task and hasattr(self._task, 'reset'):
      self._task.reset(self)   # 让仿真中的nao重合上ref中的nao
  ...
  def step(self, action):
    ...
    if self._is_render:
      ...
      if self._show_reference_id>=0:
        alpha = self._pybullet_client.readUserDebugParameter(self._show_reference_id)  # read reference on right side
      ...
      delay = self._pybullet_client.readUserDebugParameter(self._delay_id)  # read delay value on right side
      if (delay>0):
        time.sleep(delay)
