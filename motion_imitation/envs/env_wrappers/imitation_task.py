...
import os
import inspect
from motion_imitation.robots import nao
...
class ImitationTask(object):
  ...
  def reset(self, env):
    ...
    if self._default_pose is None or self._env.hard_reset:
      self._default_pose = self._record_default_pose()   # 给到nao初始状态
    ...
    self._reset_ref_motion()  # update ref nao
    ...
    self._sync_sim_model(perturb_state)  # update sim nao
  ...
  def _calc_reward_end_effector(self):
    ...
    for j in range(num_joints):
      is_end_eff = (j in robot._leg_link_ids)
      if (is_end_eff):
  ...
  def _build_ref_model(self):
    ...
    ref_model = pyb.loadURDF(urdf_file, nao.INIT_POSITION, [0, 0, 0, 1], useFixedBase=True,
                             flags=pyb.URDF_USE_MATERIAL_COLORS_FROM_MTL)
    ...
    num_joints_sim = pyb.getNumJoints(self._env.robot.biped)
    assert (
  ...
  def _build_joint_data(self):
    ...
    num_joints = self._get_num_joints()
    self._joint_pose_idx = np.zeros(len(nao.MOTOR_NAMES), dtype=np.int32)
    self._joint_pose_size = np.zeros(len(nao.MOTOR_NAMES), dtype=np.int32)
    self._joint_vel_idx = np.zeros(len(nao.MOTOR_NAMES), dtype=np.int32)
    self._joint_vel_size = np.zeros(len(nao.MOTOR_NAMES), dtype=np.int32)
    ...
    for j in range(num_joints):
      ...
      if joint_name in nao.MOTOR_NAMES:
        ...
        if (j_pose_idx < 0):
          ...
          if (i == 0):
            ...
            j_pose_idx = self._joint_pose_idx[i - 1] + self._joint_pose_size[i - 1]
            j_vel_idx = self._joint_vel_idx[i - 1] + self._joint_vel_size[i - 1]

        self._joint_pose_idx[i] = j_pose_idx
        self._joint_pose_size[i] = j_pose_size
        self._joint_vel_idx[i] = j_vel_idx
        self._joint_vel_size[i] = j_vel_size
        i += 1
  ...
  def _reset_ref_motion(self):
    ...
    self._update_ref_model()  # reset ref robot
  ...
  def _sync_sim_model(self, perturb_state):
    ...
    self._set_state(self._env.robot.biped, pose, vel)
  ...
  def _set_state(self, phys_model, pose, vel):
    ...
    num_joints = self._get_num_joints()
    idx = motion_data.MotionData.POS_SIZE + motion_data.MotionData.ROT_SIZE
    for j in range(num_joints):
      joint_info = pyb.getJointInfo(phys_model, j)
  ...
  def get_pose_size(self):
    ...
    num_joints = len(nao.MOTOR_NAMES)
    pose_size = self._get_joint_pose_idx(
        num_joints - 1) + self._get_joint_pose_size(num_joints - 1) - 8
    return pose_size
  ...
  def get_vel_size(self):
    ...
    vel_size = self._get_joint_vel_idx(
        num_joints - 1) + self._get_joint_vel_size(num_joints - 1) - 7
    return vel_size

  def _get_sim_base_position(self):
    pyb = self._get_pybullet_client()
    pos = pyb.getBasePositionAndOrientation(self._env.robot.biped)[0]
    pos = np.array(pos)
    return pos

  def _get_sim_base_rotation(self):
    pyb = self._get_pybullet_client()
    rotation = pyb.getBasePositionAndOrientation(self._env.robot.biped)[1]
