...
def build_nao_env(motor_control_mode, enable_rendering):
    ...
    sensors = [
        robot_sensors.MotorAngleSensor(num_motors=nao.NUM_MOTORS),
        robot_sensors.IMUSensor(),
        environment_sensors.LastActionSensor(num_actions=nao.NUM_MOTORS)
    ]
...
def build_imitation_env(motion_files, num_parallel_envs, mode,
  ...
  sensors = [
      sensor_wrappers.HistoricSensorWrapper(wrapped_sensor=robot_sensors.MotorAngleSensor(num_motors=nao.NUM_MOTORS), num_history=3),
      sensor_wrappers.HistoricSensorWrapper(wrapped_sensor=robot_sensors.IMUSensor(), num_history=3),
      sensor_wrappers.HistoricSensorWrapper(wrapped_sensor=environment_sensors.LastActionSensor(num_actions=nao.NUM_MOTORS), num_history=3)
  ]
  ...
  randomizers = []
  if enable_randomizer:  # 使能环境随机化以提高迁移到真实世界的鲁棒性
  ...
  env = observation_dictionary_to_array_wrapper.ObservationDictionaryToArrayWrapper(env)
  env = trajectory_generator_wrapper_env.TrajectoryGeneratorWrapperEnv(env,
                                                                       trajectory_generator=simple_openloop.NaoPoseOffsetGenerator(
                                                                           action_limit_upper=nao.UPPER_BOUND, action_limit_lower=nao.LOWER_BOUND))
